...
router.post("/", async (req, res, next) => {
  try {
    ...
    const result = await db.execute({
      sql: `INSERT INTO bean_inventory (productId, customName, customRoaster, gramsRemaining, purchaseDate, openedDate, notes, userId, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [productId || null, customName || null, customRoaster || null,
            gramsRemaining ?? 0, purchaseDate || null, openedDate || null, notes || null, userId, now, now]
    });
    const { rows } = await db.execute({
      sql: `SELECT bi.*,
        COALESCE(p.name, bi.customName, 'Unknown Bean') as displayName,
...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    // Ownership check: if row belongs to another user, deny
    if (req.user && existing.userId && existing.userId !== req.user.id) {
      return res.status(403).json({ error: "Not authorized to edit this bean" });
    }
    const b = req.body;
...
router.delete("/:id", async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(id)) return res.status(400).json({ error: "Invalid id" });
    // Ownership check
    const { rows: existingRows } = await db.execute({ sql: "SELECT userId FROM bean_inventory WHERE id = ?", args: [id] });
    if (!existingRows[0]) return res.status(404).json({ error: "Bean not found" });
    if (req.user && existingRows[0].userId && existingRows[0].userId !== req.user.id) {
      return res.status(403).json({ error: "Not authorized to delete this bean" });
    }
    await db.execute({ sql: "DELETE FROM bean_inventory WHERE id = ?", args: [id] });
    res.status(204).send();
  } catch (err) { next(err); }
});
